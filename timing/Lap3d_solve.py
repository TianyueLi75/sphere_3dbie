...
def test(lmax: int):
    ...
    # The coeff->coeff operator (bio_onsurf_apply_cplx) is pure JAX but driven eagerly with
    # scipy.sparse.linalg.gmres; the accuracy eval below (bio_offsurf_apply_cplx) is cplx, so the
    # whole test stays cplx.
    qlm_bc = sh.analys_cplx_jax(BC_pot)
    Nc = sh.nlm_cplx
    bc_np = np.asarray(qlm_bc, dtype=np.complex128)

    def matvec(xc):
        y = bio_onsurf_apply_cplx(jnp.asarray(xc), sh, sl_scal, dl_scal, sgn)
    ...
    # Warmup: compile the matvec once (untimed) before the timed solve.
    jax.block_until_ready(jnp.asarray(matvec(np.zeros((Nc,), dtype=np.complex128))))
    tstart = time.time()
    try:
        sol, info = spla.gmres(A, bc_np, rtol=1e-10, atol=1e-12, maxiter=200, callback=counter)
    except TypeError:
        # Older SciPy: `tol` instead of `rtol`.
        sol, info = spla.gmres(A, bc_np, tol=1e-10, atol=1e-12, maxiter=200, callback=counter)
    tend = time.time()
    time_gmres = tend - tstart

    sig_gmres = jnp.asarray(sol)                 # SH coefficients
    bc_check = bio_onsurf_apply_cplx(sig_gmres, sh, sl_scal, dl_scal, sgn)
    resid_gmres = jnp.linalg.norm(bc_check - qlm_bc)
    jax.debug.print("Residual of GMRES solve = {a}, number of iterations = {b}", a=resid_gmres, b=counter.count)
